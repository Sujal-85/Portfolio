...
export default function Navbar() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      lastY.current = y;
      if (pathname === '/') {
        const sections = ['#about', '#skills', '#experience'].map(s => document.querySelector(s)).filter(Boolean);
        const cur = sections.find(s => {
          const r = s.getBoundingClientRect();
          return r.top <= 140 && r.bottom > 140;
        });
        setActiveHash(cur ? '#' + cur.id : '');
      } else setActiveHash('');
    };
  ...
  const isActive = (link) => {
    if (link.hash) return pathname === '/' && activeHash === link.hash;
  ...
  return (
    ...
    >
      <nav className="max-w-7xl mx-auto flex items-center justify-between px-6 lg:px-10 h-16 md:h-20">
        ...
        <ul className="hidden lg:flex items-center gap-1">
          {navLinks.map(l => (
            <li key={l.href}>
              {l.hash ? (
                <Link to={l.href} className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                  isActive(l) ? 'text-white bg-white/[0.06]' : 'text-white/65 hover:text-white'
                }`}>{l.label}</Link>
              ) : (
                <NavLink to={l.href} className={({ isActive: a }) => `px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                  a ? 'text-white bg-white/[0.06]' : 'text-white/65 hover:text-white'
                }`}>{l.label}</NavLink>
              )}
            </li>
          ))}
        ...
        <Link to="/contact" className="hidden lg:inline-flex btn-ghost !py-2.5 !px-5 text-sm">
          Let&apos;s Talk
        </Link>
      ...
      <div className={`lg:hidden overflow-hidden transition-[max-height] duration-500 ${open ? 'max-h-[480px]' : 'max-h-0'}`}>
